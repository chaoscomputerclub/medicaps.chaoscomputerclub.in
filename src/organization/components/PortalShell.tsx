          ...
          {links.map((item) => {
            ...
            return (
              <Link
                ...
                to={item.to}
                onClick={() => dispatch(setSidebarOpen(false))}
                className={`flex items-center gap-2.5 px-2.5 py-2 rounded-md text-xs font-mono transition-colors duration-150 ${
                  active
        ...
        <div className="pt-3 border-t border-white/8 flex items-center justify-between gap-2">
          ...
          >
            <Avatar className="w-7 h-7 rounded-full border border-white/12 bg-black text-lime-400 shrink-0">
              ...
              <AvatarFallback className="rounded-full bg-lime-400/10 text-lime-400 font-mono font-bold text-[10px]">
                {initials}
              </AvatarFallback>
            </Avatar>
            <div className="min-w-0">
              <span className="block font-mono text-xs font-medium text-white truncate">
                {formattedName || displayHandle}
              </span>
              <span className="block font-mono text-[10px] text-lime-400 truncate tabular-nums">
