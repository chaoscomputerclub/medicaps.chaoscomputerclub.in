...
export const Route = createFileRoute("/portal/contests/$contestSlug/final-results")({
  validateSearch: (search: Record<string, unknown>): { state?: string } => ({
    state: search["state"] ? String(search["state"]) : "default",
  }),
  head: () => ({
    meta: [
      { title: "Final Results & Winners — CCC Medi-Caps Contests" },
      {
        name: "description",
        content:
          "Verified standings from the offline campus final: winners, scores, solved problems and rating changes.",
      },
      { property: "og:title", content: "Final Results — CCC Medi-Caps" },
      {
        property: "og:description",
        content: "Winners and verified standings from the on-campus final round.",
      },
  ...
  loader: ({ context, params }) =>
    context.queryClient.ensureQueryData(contestQueries.finalStandings(params.contestSlug)),
  component: FinalResultsPage,
});

function FinalResultsPage() {
  const { contestSlug } = Route.useParams();
  const { data: rows } = useSuspenseQuery(contestQueries.finalStandings(contestSlug));
