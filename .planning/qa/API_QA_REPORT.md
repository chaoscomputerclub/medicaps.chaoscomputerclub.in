...
> **Audit ID**: `QA-20260921-224928`  
> **Timestamp (UTC)**: `2026-09-21T22:49:29.153015+00:00`  
> **Target Base URL**: `http://testserver`  
> **Overall Success Rate**: **`100.0%`** (51/51 Tests Passed)  
> **Total Execution Latency**: `347.88ms`  
...
| :--- | :---: | :---: | :---: | :---: | :---: |
| 🟢 **System & Health** | 1 | 1 | 0 | **100.0%** | 37.44ms |
| 🟢 **Authentication** | 12 | 12 | 0 | **100.0%** | 3.11ms |
...
| 🟢 **Versioned v1 Gateway** | 3 | 3 | 0 | **100.0%** | 10.62ms |
| 🟢 **Dynamic Contest Engine** | 7 | 7 | 0 | **100.0%** | 6.08ms |
| 🟢 **Assessment Service** | 1 | 1 | 0 | **100.0%** | 4.7ms |
| 🟢 **Validation & Edge Cases** | 2 | 2 | 0 | **100.0%** | 0.99ms |
...
- **Category**: `System & Health` | **Latency**: `37.44ms`
...
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. All 4 schema keys present in response object (Latency: 37.44ms)
...
- **Endpoint**: `GET /api/auth/me`
- **Category**: `Authentication` | **Latency**: `9.89ms`
- **Expected Status**: `HTTP 200` ➔ **Actual Status**: `HTTP 200`
...
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. All 3 schema keys present in response object (Latency: 9.89ms)

### ✅ PASS `[AUTH-07]` Student Public Profile View (/profile/{handle})
- **Endpoint**: `GET /api/auth/profile/qa_organizer`
- **Category**: `Authentication` | **Latency**: `9.24ms`
- **Expected Status**: `HTTP 200` ➔ **Actual Status**: `HTTP 200`
...
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. All 4 schema keys present in response object (Latency: 9.24ms)
...
- **Category**: `Contests` | **Latency**: `12.52ms`
- **Expected Status**: `HTTP 200` ➔ **Actual Status**: `HTTP 200`
...
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. All 8 schema keys verified in list items (Latency: 12.52ms)

### ✅ PASS `[CONTEST-02]` Get Specific Contest Overview (weekly-contest-1)
- **Endpoint**: `GET /api/contests/weekly-contest-1` `[Cache: MISS]`
- **Category**: `Contests` | **Latency**: `4.08ms`
...
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. No specific schema required (Latency: 4.08ms)

### ✅ PASS `[CONTEST-03]` Get Contest Problem Arena (weekly-contest-1)
- **Endpoint**: `GET /api/contests/weekly-contest-1/problems` `[Cache: MISS]`
- **Category**: `Contests` | **Latency**: `2.65ms`
- **Expected Status**: `HTTP [200, 404]` ➔ **Actual Status**: `HTTP 200`
- **Expected Schema**: `Any valid response`
...
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. All 6 schema keys verified in list items (Latency: 4.79ms)
...
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. All 4 schema keys verified in list items (Latency: 1.76ms)
...
- **Category**: `Scoreboards` | **Latency**: `1.78ms`
- **Expected Status**: `HTTP [200, 404]` ➔ **Actual Status**: `HTTP 200`
- **Expected Schema**: `Any valid response`
- **Actual Response**: `List [0 items]. First item keys: primitive`
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. No specific schema required (Latency: 1.78ms)

### ✅ PASS `[PROOF-01]` List Cryptographic Trust Proofs
- **Endpoint**: `GET /api/verify/proofs` `[Cache: MISS]`
- **Category**: `Trust of Proof` | **Latency**: `3.54ms`
- **Expected Status**: `HTTP 200` ➔ **Actual Status**: `HTTP 200`
- **Expected Schema**: `Keys: ['certificate_id', 'sha256_digest', 'issued_at', 'status']`
- **Actual Response**: `List [0 items]. First item keys: primitive`
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. Valid (empty list returned as expected) (Latency: 3.54ms)

### ✅ PASS `[PROOF-02]` Tamper-Proof Verification Query (Invalid Token)
- **Endpoint**: `POST /api/verify`
- **Category**: `Trust of Proof` | **Latency**: `1.45ms`
- **Expected Status**: `HTTP 200` ➔ **Actual Status**: `HTTP 200`
...
- **Category**: `Social & OG` | **Latency**: `7.78ms`
- **Expected Status**: `HTTP 200` ➔ **Actual Status**: `HTTP 200`
- **Expected Schema**: `Keys: ['count', 'students']`
- **Actual Response**: `Dict with keys: ['count', 'followers_count', 'following_count', 'students']`
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. All 2 schema keys present in response object (Latency: 7.78ms)
...
- **Category**: `Versioned v1 Gateway` | **Latency**: `29.84ms`
- **Expected Status**: `HTTP 200` ➔ **Actual Status**: `HTTP 200`
...
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. All 3 schema keys present in response object (Latency: 29.84ms)
...
- **Assertion Result**: ✓ Status HTTP 201 matched expectation. All 3 schema keys present in response object (Latency: 8.66ms)
```json
// Request Body
{
  "contest_type": "weekly",
...
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. All 3 schema keys present in response object (Latency: 5.97ms)
```json
// Request Body
{
  "problem_index": "D",
...
- **Category**: `Dynamic Contest Engine` | **Latency**: `2.85ms`
- **Expected Status**: `HTTP 200` ➔ **Actual Status**: `HTTP 200`
- **Expected Schema**: `Keys: ['success', 'contest']`
- **Actual Response**: `Dict with keys: ['success', 'message', 'contest']`
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. All 2 schema keys present in response object (Latency: 2.85ms)
```json
// Request Body
{
...
- **Expected Schema**: `Keys: ['success', 'slug', 'contest_id']`
- **Actual Response**: `Dict with keys: ['success', 'message', 'contest_id', 'slug', 'title', 'cadence', 'edition', 'status']`
- **Assertion Result**: ✓ Status HTTP 201 matched expectation. All 3 schema keys present in response object (Latency: 5.41ms)
```json
// Request Body
{
  "new_title": "CCC Weekly Contest 999 \u2014 Edition 2",
  "new_slug": "weekly-contest-999-v2",
  "starts_at": "2026-09-28T22:49:28.917169+00:00",
  "ends_at": "2026-09-29T01:49:28.917178+00:00"
}
```
...
- **Category**: `Assessment Service` | **Latency**: `4.7ms`
...
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. No specific schema required (Latency: 4.7ms)
