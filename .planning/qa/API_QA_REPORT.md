...
> **Audit ID**: `QA-20260922-113718`  
> **Timestamp (UTC)**: `2026-09-22T11:37:19.061022+00:00`  
> **Target Base URL**: `http://testserver`  
> **Overall Success Rate**: **`100.0%`** (51/51 Tests Passed)  
> **Total Execution Latency**: `384.46ms`  
...
| 🟢 **Authentication** | 12 | 12 | 0 | **100.0%** | 4.64ms |
...
| 🟢 **Campus Passes** | 2 | 2 | 0 | **100.0%** | 3.88ms |
| 🟢 **Campus Feed** | 2 | 2 | 0 | **100.0%** | 2.96ms |
| 🟢 **Social & OG** | 9 | 9 | 0 | **100.0%** | 3.04ms |
| 🟢 **Versioned v1 Gateway** | 3 | 3 | 0 | **100.0%** | 10.85ms |
| 🟢 **Dynamic Contest Engine** | 7 | 7 | 0 | **100.0%** | 8.38ms |
| 🟢 **Assessment Service** | 1 | 1 | 0 | **100.0%** | 5.61ms |
...
- **Endpoint**: `GET /api/auth/me`
- **Category**: `Authentication` | **Latency**: `15.72ms`
- **Expected Status**: `HTTP 200` ➔ **Actual Status**: `HTTP 200`
...
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. All 3 schema keys present in response object (Latency: 15.72ms)

### ✅ PASS `[AUTH-07]` Student Public Profile View (/profile/{handle})
- **Endpoint**: `GET /api/auth/profile/qa_organizer`
- **Category**: `Authentication` | **Latency**: `19.24ms`
- **Expected Status**: `HTTP 200` ➔ **Actual Status**: `HTTP 200`
...
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. All 4 schema keys present in response object (Latency: 19.24ms)
...
- **Actual Response**: `Dict with keys: ['member', 'ratingHistory', 'history', 'recentBattles', 'battles', 'problemStats', 'submissionCalendar', 'proofs']`
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. All 3 schema keys present in response object (Latency: 3.04ms)
...
- **Category**: `Contests` | **Latency**: `12.07ms`
- **Expected Status**: `HTTP 200` ➔ **Actual Status**: `HTTP 200`
...
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. All 8 schema keys verified in list items (Latency: 12.07ms)
...
- **Category**: `Leaderboards & Ratings` | **Latency**: `4.72ms`
- **Expected Status**: `HTTP 200` ➔ **Actual Status**: `HTTP 200`
...
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. All 6 schema keys verified in list items (Latency: 4.72ms)

### ✅ PASS `[LEADER-02]` Departmental Aggregate Ratings
- **Endpoint**: `GET /api/leaderboard/departments` `[Cache: MISS]`
- **Category**: `Leaderboards & Ratings` | **Latency**: `1.74ms`
- **Expected Status**: `HTTP 200` ➔ **Actual Status**: `HTTP 200`
...
- **Category**: `Campus Passes` | **Latency**: `6.6ms`
- **Expected Status**: `HTTP [200, 404]` ➔ **Actual Status**: `HTTP 200`
- **Expected Schema**: `Any valid response`
...
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. No specific schema required (Latency: 6.6ms)
...
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. All 1 schema keys present in response object (Latency: 1.98ms)
...
- **Category**: `Social & OG` | **Latency**: `5.42ms`
- **Expected Status**: `HTTP 200` ➔ **Actual Status**: `HTTP 200`
...
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. All 6 schema keys present in response object (Latency: 5.42ms)

### ✅ PASS `[SOCIAL-08]` Atomic Toggle Unfollow Peer Cadet (POST /social/toggle second pass)
- **Endpoint**: `POST /api/social/toggle/qa_target`
- **Category**: `Social & OG` | **Latency**: `3.98ms`
- **Expected Status**: `HTTP 200` ➔ **Actual Status**: `HTTP 200`
- **Expected Schema**: `Keys: ['success', 'is_following', 'followers_count', 'following_count']`
- **Actual Response**: `Dict with keys: ['success', 'is_following', 'followers_count', 'following_count', 'target_id', 'target_handle', 'message']`
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. All 4 schema keys present in response object (Latency: 3.98ms)
...
- **Category**: `Versioned v1 Gateway` | **Latency**: `30.49ms`
- **Expected Status**: `HTTP 200` ➔ **Actual Status**: `HTTP 200`
...
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. All 3 schema keys present in response object (Latency: 30.49ms)

### ✅ PASS `[V1-02]` v1 Contests Resource Gateway
- **Endpoint**: `GET /api/v1/contests` `[Cache: HIT]`
- **Category**: `Versioned v1 Gateway` | **Latency**: `1.16ms`
- **Expected Status**: `HTTP 200` ➔ **Actual Status**: `HTTP 200`
- **Expected Schema**: `Keys: ['id', 'title', 'slug', 'status']`
- **Actual Response**: `List [1 items]. First item keys: ['id', 'slug', 'title', 'season', 'status', 'division', 'starts_at', 'ends_at']`
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. All 4 schema keys verified in list items (Latency: 1.16ms)
...
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. All 3 schema keys present in response object (Latency: 5.41ms)
```json
// Request Body
{
  "problem_index": "D",
...
- **Category**: `Dynamic Contest Engine` | **Latency**: `11.39ms`
- **Expected Status**: `HTTP 200` ➔ **Actual Status**: `HTTP 200`
...
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. All 2 schema keys present in response object (Latency: 11.39ms)
```json
// Request Body
{
  "status": "live"
...
- **Category**: `Dynamic Contest Engine` | **Latency**: `15.33ms`
- **Expected Status**: `HTTP 201` ➔ **Actual Status**: `HTTP 201`
- **Expected Schema**: `Keys: ['success', 'slug', 'contest_id']`
- **Actual Response**: `Dict with keys: ['success', 'message', 'contest_id', 'slug', 'title', 'cadence', 'edition', 'status']`
- **Assertion Result**: ✓ Status HTTP 201 matched expectation. All 3 schema keys present in response object (Latency: 15.33ms)
```json
// Request Body
{
  "new_title": "CCC Weekly Contest 999 \u2014 Edition 2",
  "new_slug": "weekly-contest-999-v2",
  "starts_at": "2026-09-29T11:37:18.783731+00:00",
  "ends_at": "2026-09-29T14:37:18.783741+00:00"
}
```

### ✅ PASS `[DYNAMIC-06]` Cascade Delete QA Contests
- **Endpoint**: `DELETE /api/admin/contests/weekly-contest-999`
- **Category**: `Dynamic Contest Engine` | **Latency**: `10.23ms`
- **Expected Status**: `HTTP 200` ➔ **Actual Status**: `HTTP 200`
- **Expected Schema**: `Keys: ['success', 'message']`
- **Actual Response**: `Dict with keys: ['success', 'message']`
- **Assertion Result**: ✓ Status HTTP 200 matched expectation. All 2 schema keys present in response object (Latency: 10.23ms)
...
- **Category**: `Assessment Service` | **Latency**: `5.61ms`
