...
def ip_rate_limit(scope: str, max_calls: int, window_seconds: int) -> Callable:
    """
    FastAPI dependency factory that enforces a sliding-window rate limit keyed by
    the authentic visitor IP address (prioritizing Cloudflare CF-Connecting-IP).
    Protects public sensitive routes such as OTP request and auth endpoints from DDoS/bot abuse.
    """
    async def _check(request: Request) -> None:
        from app.core.redis import get_redis
        from app.core.cloudflare import get_client_ip

        client_ip = get_client_ip(request)
        key = f"ccc:rl:ip:{scope}:{client_ip}"
        now_ms = int(time.time() * 1000)
        window_ms = window_seconds * 1000
        ...
        try:
            ...
            if current_count >= max_calls:
                raise HTTPException(
                    status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                    detail=f"Rate limit exceeded: maximum {max_calls} attempts allowed per {window_seconds}s.",
                    headers={"Retry-After": str(window_seconds)},
                )
        except HTTPException:
            raise
        except Exception as exc:
            logger.warning("ip_rate_limit: Redis error for key '%s': %s — allowing through", key, exc)
